        ...
        if ($request->ajax()) {
            return DataTables::of($usertypes)
                ->addColumn('actions', function ($usertype) {
                    // Predefinir los tipos de usuario que no deben tener acciones
                    $predefinedTypes = ['Administrador', 'Conductor', 'Recolector', 'Ciudadano'];

                    // Verificar si el tipo de usuario actual está en la lista de tipos predefinidos
                    if (in_array($usertype->name, $predefinedTypes)) {
                        // Si es un tipo de usuario bloqueado, no mostrar las acciones
                        return '<span class="text-muted">Acciones deshabilitadas</span>';
                    }

                    // Si no es un tipo bloqueado, mostrar las acciones
                    return '
                    <div class="dropdown">
